...
public class customer extends Application {
    ...
    ObservableList<company.Person> data1 =company.data1; //create observable liste from class company (type)
  ...
  t11.setText("");
    ...
    class EditingCell extends TableCell<Person, String> {
        ...
        String selectSQL = "UPDATE `customer` SET `fname`=?,`lname`=?,`company`=?,`phone`=?,`str1`=?,`str2`=?,`city`=?,"
                + "`state`=?,`zip`=?,`country`=?,`fax`=?,`type`=? WHERE idcustomer=?";
    ...
    void selectCustomerFrom(){
        ...
        try {
            ...
            while (rs.next()) {
              data.add(new Person(String.valueOf(rs.getInt("idcustomer")),rs.getString("fname"), rs.getString("lname"), 
